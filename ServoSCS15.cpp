...
// Tag pour les logs
static const char *TAG = "SCS15";

// Structure pour passer les paramètres à la tâche
struct MotorTaskParams {
    SCS15 *controller;  // Référence à l'objet SCS15
    uint8_t motor_id;
...
// Constructeur
SCS15::SCS15(uart_port_t uart_num, int tx_pin, int rx_pin, int baud_rate)
...
void SCS15::send_command(const uint8_t *data, size_t len) {
    if (uart_mutex_ == NULL) {
        ESP_LOGE(TAG, "UART mutex is NULL, cannot send command");
        return;  // Ne rien faire si le mutex est NULL
    }

    // Protéger l'accès à l'UART avec un mutex
    if (xSemaphoreTake(uart_mutex_, portMAX_DELAY)) {
        uart_write_bytes(uart_num_, (const char *)data, len);
        ESP_LOGI(TAG, "Commande envoyée");
        xSemaphoreGive(uart_mutex_);  // Libérer le mutex après l'envoi
    } else {
...
void SCS15::change_motor_id(uint8_t current_id, uint8_t new_id) {
//     uint8_t command[] = {
...
// }
    uint8_t command[6];
    command[0] = 0xFF;  // Header pour la commande SCS15
    command[1] = 0xFF;  // Header pour la commande SCS15
    command[2] = current_id;  // ID actuel du servomoteur
    command[3] = 0x03;  // Longueur du paquet
    command[4] = 0x03;  // Commande d'écriture
    command[5] = new_id;  // Nouvel ID

    // Calcul du checksum (XOR de tous les octets sauf les deux premiers)
    uint8_t checksum = command[2] ^ command[3] ^ command[4] ^ command[5];
    uart_write_bytes(uart_num_, (const char *)command, 6);
    uart_write_bytes(uart_num_, (const char *)&checksum, 1);  // Envoyer le checksum
    ESP_LOGI(TAG, "Commande de changement d'ID envoyée: ID actuel %d, Nouvel ID %d", current_id, new_id);
}

// Méthode pour pinguer un moteur
bool SCS15::ping_motor(uint8_t id) {
...
void SCS15::move_motor_only(uint8_t id, uint16_t position, uint16_t speed) {
    if (uart_mutex_ == NULL) {
...
int SCS15::move_and_get_position(uint8_t id, uint16_t position, uint16_t speed) {
    if (uart_mutex_ == NULL) {
        ESP_LOGE(TAG, "UART mutex is NULL, cannot move motor");
...
void SCS15::motor_task(void *pvParameters) {
...
void SCS15::motor_move_only_task(void *pvParameters) {
    MotorTaskParams *params = (MotorTaskParams *)pvParameters;
...
bool SCS15::start_motor_thread(uint8_t id, uint16_t initial_position, uint16_t initial_speed, const char* task_name) {
...
// Méthode pour démarrer un thread pour contrôler un moteur
bool SCS15::start_motor_move_only_thread(uint8_t id, uint16_t initial_position, uint16_t initial_speed, const char* task_name) {
    MotorTaskParams *params = (MotorTaskParams *)malloc(sizeof(MotorTaskParams));  // Utiliser malloc pour l'allocation
    if (params == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for MotorTaskParams");
        return false;
